...
async def getPosts(token: str = Depends(oauth2_scheme)):
...
async def getPostById(id: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
...
async def getPostsByCreator(creator: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
...
async def addPost(post: Post, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
        ...
        try:
            ...
            await users.updateUser(search_user, token)
            logging.info(
...
async def updatePost(post: Post, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
        ...
        if post.__eq__(post_search):
            logging.info(
                f"The post '{post.id}' has not been changed so no update required"
            )
            raise HTTPException(
                status_code=status.HTTP_204_NO_CONTENT,
        ...
        try:
            user_search = await users.getUserByUsername(post_search.creator, token)
...
async def deletePost(id: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
        ...
        try:
            search_user = await users.getUserByUsername(post_search.creator, token)

            logging.info(f"Deleting post comments")
            await comments.deleteAllPostComments(id, token)
...
async def deleteAllCreatorPosts(creator: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
        ...
        if len(result) == 0:
            logging.info(
                f"The user {creator} has no posts, so, anything will be deleted"
            )
            # raise HTTPException(
            #     status_code=status.HTTP_404_NOT_FOUND,
            ...
            for post in post_search:
                posts_list.append(await deletePost(post["id"], token))
