...
async def getExercisesTO(token: str = Depends(oauth2_scheme)):
...
async def getExerciseTOById(id: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
...
async def getExercisesTOByCreator(creator: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
...
async def getExerciseTOByName(name: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
...
async def addExerciseTO(exerciseTO: ExerciseTO, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
        ...
        if (
            type(await searchExerciseTO(sql_cursor, "name", exerciseTO.name))
            == ExerciseTO
        ):
        ...
        logging.info(
            "The exercise does not exist in the database and is being processed"
        )
...
async def updateExerciseTO(exerciseTO: ExerciseTO, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
...
async def deleteExerciseTO(id: str, token: str = Depends(oauth2_scheme)):
    if token_validation(token) != None:
