...
from src.db.mongodb.schemas.comment import comments_schema, comment_schema
...
async def addCommentToPost(postId: str, comment: Comment):
    ...
    try:
        ...
        await deleteCommentFromPost(postId, ObjectId(id))
...
async def updateCommentFromPost(postId: str, comment: Comment):
    ...
    try:
        ...
        post = await posts.updatePost(search_post)
...
async def deleteCommentFromPost(postId: str, commentId: str):
    ...
    try:
        await mongodb_client.comments.find_one_and_delete({"_id": ObjectId(commentId)})
