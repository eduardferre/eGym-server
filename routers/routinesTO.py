...
from ddbb.sqlDB.schemas.routineTO import routineTO_schema
...
@router.get("/creator/{creator}", response_model=list[RoutineTO], status_code=status.HTTP_200_OK)
...
@router.get("/routineName/{name}", response_model=list[RoutineTO], status_code=status.HTTP_200_OK)
...
@router.post("/{routineId}_{exerciseId}", response_model=RoutineTO, status_code=status.HTTP_201_CREATED)
async def addExerciseTOToRoutineTO(routineId: int, exerciseId: int):
    ...
    if exercise == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"The exercise with id = {exerciseId} does not exist")
    ...
    routine = sql_cursor.fetchone()
    if routine == None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"The routine with id = {routineId} does not exist")
    ...
    if relation != None:
        raise HTTPException(status_code=status.HTTP_409_CONFLICT, detail="The exercise is already in the routine")
...
def searchRoutineExercises(routineId: int): # A way to populate RoutineTO object with ExerciseTO objects
